	...
	try
	{
		...
		{	
			...
			else
			{
				...
				std::string fullpath = std::string(path) + std::string("/Csound_Hadron/plugins64");
				result = csoundSetGlobalEnv("OPCODEDIR64", fullpath.c_str());
				...
				result = csoundSetGlobalEnv("INCDIR", path);
				
				incdir = (char*) csoundGetEnv(NULL, "INCDIR");
				if(!incdir)
					m_msg_buf.addv(message::_ERROR_MSG, "csoundSetGlobalEnv(INCDIR,%s) failed.", path);
			...
			if(CSOUND_SUCCESS != csoundPreCompile(m_csound))
				m_msg_buf.add(message::_ERROR_MSG, "csoundPreCompile() failed.");
			else
			{
				#ifdef _WINDOWS
...
void inputValueCallback(CSOUND *csound, const char *name, MYFLT *val)
{
	...
	if(cso->m_x->input) cso->m_iChanGroup.GetVal(name, val);
}
...
void outputValueCallback(CSOUND *csound, const char *name, MYFLT val)
{
	...
	x->cso->m_oChanGroup.SetVal(name, type, val);
}
