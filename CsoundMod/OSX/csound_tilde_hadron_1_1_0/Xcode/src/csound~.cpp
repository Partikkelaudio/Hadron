...
int T_EXPORT main(void)
{
	...
	c = class_new("csound~_hadron", (method)csound_new, (method)csound_free, (long)sizeof(t_csound), 0L, A_GIMME, 0);
...
void *csound_new(t_symbol *s, short argc, t_atom *argv)
{
	...
	try
	{
		...
		if(firstTime)
		{
			...
			object_post(x->m_obj, "csound~_hadron v1.1.0");
			firstTime = false;
		}
